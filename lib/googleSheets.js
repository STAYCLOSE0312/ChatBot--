...
class GoogleSheetsService {
  ...
  async init() {
    try {
      ...
      // Base64エンコードされた認証情報を優先的に読み込む（Render用・推奨）
      if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON_BASE64) {
        console.log('📝 Base64エンコードされた認証情報を読み込みます...');
        try {
          // Base64デコード
          const jsonString = Buffer.from(process.env.GOOGLE_SERVICE_ACCOUNT_JSON_BASE64, 'base64').toString('utf8');
          ...
          if (!keyData.type || !keyData.project_id || !keyData.private_key || !keyData.client_email) {
            throw new Error('認証情報JSONに必須フィールドが不足しています');
          }
          
          console.log(`   ✅ 認証情報を読み込みました (project_id: ${keyData.project_id})`);
          ...
          console.error('❌ Base64認証情報の解析エラー:', parseError.message);
          throw new Error(`認証情報JSONの解析に失敗しました: ${parseError.message}`);
        }
      }
      // 環境変数から認証情報を読み込む（Render用・フォールバック）
        ...
        try {
          ...
          console.error('❌ 認証情報JSONの解析エラー:', parseError.message);
          console.error('   JSONの最初の200文字:', process.env.GOOGLE_SERVICE_ACCOUNT_JSON?.substring(0, 200));
          console.error('   ヒント: Base64エンコードを使用する場合は、GOOGLE_SERVICE_ACCOUNT_JSON_BASE64環境変数を設定してください');
          throw new Error(`認証情報JSONの解析に失敗しました: ${parseError.message}`);
        }
      }
      // ファイルパスから読み込む（ローカル環境用）
        ...
        throw new Error('認証情報が設定されていません。GOOGLE_SERVICE_ACCOUNT_JSON_BASE64、GOOGLE_SERVICE_ACCOUNT_JSON または GOOGLE_SERVICE_ACCOUNT_PATH を設定してください。');
      }
  ...
  async ensureInitialized() {
    ...
    if (this.initPromise) {
      try {
        ...
        // 初期化後もthis.sheetsがnullの場合はエラー
        if (!this.sheets) {
          throw new Error('Google Sheets APIの初期化に失敗しました。認証情報を確認してください。');
        }
      } catch (error) {
        // エラーを再スローして、呼び出し元で適切に処理できるようにする
        console.error('❌ ensureInitialized: 初期化エラーが発生しました:', error.message);
        throw error;
      }
    } else {
